...
public abstract class NimSdkType implements NimSdkTypeId {
  public static final ExtensionPointName<NimSdkType> EP_NAME = ExtensionPointName.create("org.nim.nim-plugin.nimSdkType");

  private static final Comparator<NimSdk> ALPHABETICAL_COMPARATOR = (sdk1, sdk2) -> StringUtil.compare(sdk1.getName(), sdk2.getName(), true);

  private final String myName;

  public NimSdkType(@NotNull String name) {
    myName = name;
  }
  ...
  @Override
  @Nullable
  public String getVersionString(@NotNull NimSdk sdk) {
   ...
   */
  @NotNull
  public Comparator<NimSdk> getComparator() {
  ...
  public void setupSdkPaths(@NotNull NimSdk sdk) {}

  /**
   * @return Configurable object for the SDKs additional data or null if not applicable
   */
  @Nullable
  public abstract NimJdkAdditionalDataConfigurable createAdditionalDataConfigurable(@NotNull NimSdkModel sdkModel, @NotNull NimSdkModificator sdkModificator);

  @Nullable
  public NimSdkAdditionalData loadAdditionalData(@NotNull Element additional) {
    return null;
  }

  @Override
  @Nullable
  public NimSdkAdditionalData loadAdditionalData(@NotNull NimSdk currentSdk, @NotNull Element additional) {
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof NimSdkType)) return false;

    NimSdkType sdkType = (NimSdkType)o;
  ...
  @NotNull
  public static NimSdkType[] getAllTypes() {
    //noinspection deprecation
    NimSdkType[] components = ApplicationManager.getApplication().getComponents(NimSdkType.class);
    List<NimSdkType> list1 = components.length == 0 ? Collections.emptyList() : Arrays.asList(components);
    return ContainerUtil.concat(list1, EP_NAME.getExtensionList()).toArray(new NimSdkType[0]);
  }

  @NotNull
  public static <T extends NimSdkType> T findInstance(@NotNull Class<T> sdkTypeClass) {
    for (NimSdkType sdkType : EP_NAME.getExtensionList()) {
   ...
   *         null otherwise
   */
  public NimSdkType getDependencyType() {
    return null;
  }
   ...
   * @implSpec method's implementations should not add sdk to the jdkTable neither invoke {@link NimSdkType#setupSdkPaths}. Only create and
  ...
  public void showCustomCreateUI(@NotNull NimSdkModel sdkModel,
                                 @NotNull JComponent parentComponent,
                                 @Nullable NimSdk selectedSdk,
                                 @NotNull Consumer<NimSdk> sdkCreatedCallback) {
    //noinspection deprecation
  ...
  public void showCustomCreateUI(@NotNull NimSdkModel sdkModel, @NotNull JComponent parentComponent, @NotNull Consumer<NimSdk> sdkCreatedCallback) { }
   ...
   */
  public boolean sdkHasValidPath(@NotNull NimSdk sdk) {
